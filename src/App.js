...
const ORAL_CATS = [
  { cat:"🎯 A — LE CADRE", color:"bg-blue-100 border-blue-300 text-blue-800", tips:[
  ...
  { cat:"👤 B — PERSONNELLE", color:"bg-purple-100 border-purple-300 text-purple-800", tips:[
  ...
  { cat:"🔥 C — VIVANTE", color:"bg-orange-100 border-orange-300 text-orange-800", tips:[
...
export default function App() {
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const txt = data.content?.[0]?.text || "Désolé, je n'ai pas pu répondre.";
      setChat(prev=>[...prev, {role:'ai', txt}]);
    } catch {
      setChat(prev=>[...prev, {role:'ai', txt:"⚠️ Erreur de connexion. Veuillez réessayer."}]);
    }
    setChatLoad(false);
  };
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:16}}>
          {MODULES.map(m=>(
            <div key={m.id} style={{background:'white',borderRadius:16,overflow:'hidden',boxShadow:'0 4px 20px rgba(0,0,0,0.15)',cursor:'pointer',transition:'transform 0.2s'}}
              ...
              <div style={{background:`linear-gradient(135deg,var(--g1),var(--g2))`,backgroundImage:`linear-gradient(135deg,${m.grad.replace('from-','').replace('to-',',')})`,padding:'20px 20px 16px'}}>
  ...
  if(page==='module' && mod) return (
    <div style={{minHeight:'100vh',background:'#f8fafc',fontFamily:'system-ui,sans-serif'}}>
      <div style={{backgroundImage:`linear-gradient(135deg,${mod.grad.includes('blue')?'#3b82f6,#1d4ed8':mod.grad.includes('orange')?'#f97316,#dc2626':mod.grad.includes('green')?'#10b981,#065f46':mod.grad.includes('purple')?'#8b5cf6,#6d28d9':mod.grad.includes('rose')?'#f43f5e,#db2777':'#14b8a6,#0e7490'})`,padding:'24px',display:'flex',alignItems:'center',gap:16}}>
        <button onClick={()=>setPage('home')} style={{background:'rgba(255,255,255,0.2)',border:'none',color:'white',width:36,height:36,borderRadius:'50%',cursor:'pointer',fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>←</button>
        <div>
  ...
  if(page==='chat') return (
    <div style={{minHeight:'100vh',background:'#f8fafc',fontFamily:'system-ui,sans-serif',display:'flex',flexDirection:'column'}}>
      <div style={{background:'linear-gradient(135deg,#6366f1,#4f46e5)',padding:'20px 24px',display:'flex',alignItems:'center',gap:16}}>
        <button onClick={()=>setPage('home')} style={{background:'rgba(255,255,255,0.2)',border:'none',color:'white',width:36,height:36,borderRadius:'50%',cursor:'pointer',fontSize:18}}>←</button>
        <div><div style={{color:'white',fontWeight:800,fontSize:20}}>💬 Assistant IA DGFiP</div><div style={{color:'rgba(255,255,255,0.8)',fontSize:13}}>Posez vos questions sur la DGFiP</div></div>
      </div>
